...
export default function Project() {
  ...
  const contactRef = sectionRefs?.contactRef as React.RefObject<HTMLElement> | undefined;
  ...
  const isProjectInView = useInView(projectSectionRef, { once: true, amount: 0.1 });
  const isHeaderInView = useInView(projectHeaderRef, { once: true, amount: 0.3 });
  ...
  const projects: ProjectItem[] = [
    {
      ...
      description: "Aplikasi modern untuk menghitung Indeks Massa Tubuh dengan rekomendasi kesehatan personal dan visualisasi data yang interaktif.",
      image: "https://images.unsplash.com/photo-1576091160550-2173dba999ef?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
      technologies: [
        { name: "HTML", icon: TechIcons.HTML.icon, color: TechIcons.HTML.color },
        { name: "CSS", icon: TechIcons.CSS.icon, color: TechIcons.CSS.color },
        { name: "PHP", icon: TechIcons.PHP.icon, color: TechIcons.PHP.color }
      ],
      liveUrl: "http://www.bayuajiys.byethost32.com/IMT/",
      category: "Web App"
    },
    {
      id: 2,
      title: "Shell Fuel Management",
      description: "Sistem manajemen bahan bakar terintegrasi dengan dashboard analytics real-time dan pelaporan otomatis untuk optimasi operasional.",
      image: "https://images.unsplash.com/photo-1542273917363-3b1817f69a2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
      technologies: [
        { name: "HTML", icon: TechIcons.HTML.icon, color: TechIcons.HTML.color },
        { name: "Bootstrap", icon: TechIcons.Bootstrap.icon, color: TechIcons.Bootstrap.color },
        { name: "PHP", icon: TechIcons.PHP.icon, color: TechIcons.PHP.color }
      ],
    ...
    },
    {
      ...
      description: "Platform rental motor dengan sistem booking intelligent, manajemen fleet otomatis, dan integrasi payment gateway untuk pengalaman seamless.",
    ...
    },
    {
      ...
      description: "Sistem manajemen data siswa dengan AI-powered analytics, tracking akademik real-time, dan dashboard interaktif untuk pendidik modern.",
    ...
    },
    {
      ...
      description: "Sistem point of sale modern dengan inventory management, sales analytics, dan multi-payment integration untuk bisnis retail masa depan.",
    ...
    },
    {
      ...
      description: "E-commerce platform dengan React architecture, responsive design, dan UX yang dioptimalkan untuk konversi tinggi dan engagement maksimal.",
      image: "https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
      technologies: [
        { name: "HTML", icon: TechIcons.HTML.icon, color: TechIcons.HTML.color },
        { name: "CSS", icon: TechIcons.CSS.icon, color: TechIcons.CSS.color },
        { name: "React", icon: TechIcons.React.icon, color: TechIcons.React.color }
      ],
  ...
  const categories = ["All", "Web App", "Management System", "Business Platform"];
    ...
    >
      ...
      <div className="absolute inset-0 opacity-30">
        <motion.div 
          ...
          animate={isProjectInView ? { opacity: 0.2, scale: 1 } : { opacity: 0, scale: 0.8 }}
      ...
      <div className="container relative px-4 mx-auto max-w-7xl">
        ...
        >
          ...
          >
            Koleksi project yang menggabungkan teknologi modern dengan solusi bisnis nyata. 
            Setiap project dirancang dengan perhatian detail untuk memberikan pengalaman pengguna yang luar biasa.
          </motion.p>
        </motion.div>
        ...
        >
          {filteredProjects.slice(0, visibleProjectCount).map((project, index) => {
            ...
            <motion.div
              ...
              animate={isGridInView ? { opacity: isFading ? 0 : 1, y: isFading ? 20 : 0 } : { opacity: 0, y: 40 }}
              transition={{ 
                duration: 0.6, 
                delay: index * 0.15,
                type: "spring",
            ...
            <div className="grid gap-0 lg:grid-cols-2">
              ...
              <div className="p-8 lg:p-10">
                ...
                <div className="pt-6 mt-8 border-t border-gray-200 dark:border-gray-700">
                  <h5 className="mb-3 font-semibold text-gray-900 dark:text-white">Key Features:</h5>
                  <div className="flex flex-wrap gap-2">
                    {selectedProject.id === 1 && (
                      <>
                        <span className="px-2 py-1 text-xs text-green-800 bg-green-100 rounded-md dark:bg-green-900/30 dark:text-green-400">Health Analytics</span>
                        <span className="px-2 py-1 text-xs text-blue-800 bg-blue-100 rounded-md dark:bg-blue-900/30 dark:text-blue-400">BMI Calculator</span>
                        <span className="px-2 py-1 text-xs text-purple-800 bg-purple-100 rounded-md dark:bg-purple-900/30 dark:text-purple-400">Recommendations</span>
                      </>
                    )}
                    {selectedProject.id === 2 && (
                      <>
                        <span className="px-2 py-1 text-xs text-yellow-800 bg-yellow-100 rounded-md dark:bg-yellow-900/30 dark:text-yellow-400">Real-time Tracking</span>
                        <span className="px-2 py-1 text-xs text-red-800 bg-red-100 rounded-md dark:bg-red-900/30 dark:text-red-400">Analytics Dashboard</span>
                        <span className="px-2 py-1 text-xs text-indigo-800 bg-indigo-100 rounded-md dark:bg-indigo-900/30 dark:text-indigo-400">Reporting</span>
                      </>
                    )}
                    {selectedProject.id === 3 && (
                      <>
                        <span className="px-2 py-1 text-xs rounded-md bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">Smart Booking</span>
                        <span className="px-2 py-1 text-xs text-orange-800 bg-orange-100 rounded-md dark:bg-orange-900/30 dark:text-orange-400">Fleet Management</span>
                        <span className="px-2 py-1 text-xs text-pink-800 bg-pink-100 rounded-md dark:bg-pink-900/30 dark:text-pink-400">Payment Gateway</span>
                      </>
                    )}
                    {selectedProject.id === 4 && (
                      <>
                        <span className="px-2 py-1 text-xs rounded-md bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-400">Student Analytics</span>
                        <span className="px-2 py-1 text-xs rounded-md bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-400">Academic Tracking</span>
                        <span className="px-2 py-1 text-xs rounded-md bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400">Interactive Dashboard</span>
                      </>
                    )}
                    {selectedProject.id === 5 && (
                      <>
                        <span className="px-2 py-1 text-xs text-teal-800 bg-teal-100 rounded-md dark:bg-teal-900/30 dark:text-teal-400">POS System</span>
                        <span className="px-2 py-1 text-xs rounded-md bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">Inventory Control</span>
                        <span className="px-2 py-1 text-xs rounded-md bg-lime-100 text-lime-800 dark:bg-lime-900/30 dark:text-lime-400">Sales Analytics</span>
                      </>
                    )}
                    {selectedProject.id === 6 && (
                      <>
                        <span className="px-2 py-1 text-xs rounded-md bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-400">React Architecture</span>
                        <span className="px-2 py-1 text-xs text-indigo-800 bg-indigo-100 rounded-md dark:bg-indigo-900/30 dark:text-indigo-400">Responsive Design</span>
                        <span className="px-2 py-1 text-xs text-purple-800 bg-purple-100 rounded-md dark:bg-purple-900/30 dark:text-purple-400">Modern UX</span>
                      </>
                    )}
                  </div>
                </div>
